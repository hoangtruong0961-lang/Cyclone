...
import { getRegexedString, extractTagContent, parseChoices, ensureThinkingTagsBalanced } from "../../../../utils/regex";
        ...
        for await (const chunk of stream) {
          ...
          if (now - lastUIUpdateTime > UI_UPDATE_INTERVAL) {
            if (targetIndex !== undefined && workingHistory[targetIndex]) {
              ...
              let displayContent = ensureThinkingTagsBalanced(accumulatedText);
